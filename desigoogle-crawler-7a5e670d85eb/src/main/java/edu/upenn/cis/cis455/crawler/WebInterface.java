...
//                        "<textarea style=\"width:100%; height:100%\" readonly>" + content + "</textarea></body></html>";
        ...
        get("/show", ((request, response) -> {
            ...
            //
            List<String> docChannelIndices = db.getIndexManager().getDocIdForChannel(userChannelInfo.getChannelName());
            List<DocOnlyInfo> docOnlyInfoList = new ArrayList<>();

            docChannelIndices.stream().forEach(entry -> {
                try {
                    DocOnlyInfo docOnlyInfo = db.getUrlDataManager().getOnlyDocOnlyInfoFromId(entry);
                    if(docOnlyInfo!=null) {
