...
public class RobotPermissionChecker implements IProcess {
    ...
    private final String DEFAULT_ROBOT_FILE = "User-agent: *\n" +
            "Crawl-delay: 1\n";
    ...
    public void processRequest(ProcessContext processContext) {
        ...
        HostRobotInfo hostRobotInfo = hostInfoExtractorService.getHostInfo(protocol, processContext.getUrlInfo().getHostName(),
                processContext.getUrlInfo().getPortNo(), null);
...
//            return;
            ...
            hostRobotInfo = new HostRobotInfo();
            hostRobotInfo.setPort(processContext.getUrlInfo().getPortNo());
            hostRobotInfo.setAdditionalInfo(new HashMap<>());
            hostRobotInfo.setRobotStructure(DEFAULT_ROBOT_FILE);
