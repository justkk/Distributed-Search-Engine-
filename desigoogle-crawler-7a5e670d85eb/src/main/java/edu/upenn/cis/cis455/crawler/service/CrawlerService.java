...
public class CrawlerService {
    ...
    public CrawlServiceResponse processRequest(ReadyQueueInstance readyQueueInstance) {
        ...
        ProcessContext processContext = new ProcessContext(readyQueueInstance.getUrlInfo(), crawler.getMaxDocumentSize());
        processContext.setParentId(readyQueueInstance.getParentId());
        try {
        ...
        if(processContext.isRedirect()) {
            return new CrawlServiceResponse(
                    Arrays.asList(new ReadyQueueInstance(processContext.getRedirectUrlInfo(),
                            readyQueueInstance.getParentId())),
    ...
    private boolean shouldIProcessDocument(ProcessContext processContext) {
        ...
        synchronized (this.crawler) {
            if(crawler.getCrawlDataStructure().isDocumentVisited(processContext.getMd5Hash())){
            ...
            crawler.getCrawlDataStructure().addDocumentVisited(processContext.getMd5Hash());
        ...
        if (docType == DocType.HTML) {
            ...
            try {
                ...
                List<ReadyQueueInstance> readyQueueInstances = urlInfoList.stream().map(urlInfo
                        -> new ReadyQueueInstance(urlInfo, String.valueOf(processContext.getUrlDataInfo().getUrlDataInfoKey().getKeyString()))).collect(Collectors.toList());
